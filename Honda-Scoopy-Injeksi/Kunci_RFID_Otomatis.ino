...
#define relay1 1     // Set Relay Pin
#define relay2 2     // Set Relay Pin
...
void setup() {
  ...
  digitalWrite(relay1, LOW);    // Make Maotor is mati
  ...
  if (programMode) {
    ...
    if ( isMaster(readCard) ) {   // If scanned card's ID matches Master Card's ID enter program mode
      ...
      if ( findID(readCard) ) { // If not, see if the card is in the EEPROM
        Serial.println(F("Selamat Datang, Kamu Boleh Naik Motornya Fiki Firmansyah"));
...
void granted () {
  ...
  digitalWrite(relay1, HIGH);     // Relay Motor Hidup
  delay(43200000);
}
...
void normalModeOn () {
  ...
  digitalWrite(redLed, LED_OFF);  // Make sure Red LED is off
  digitalWrite(greenLed, LED_OFF);  // Make sure Green LED is off
  digitalWrite(relay1, LOW);    // Make Sure motor relay mati
}
